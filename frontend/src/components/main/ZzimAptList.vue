...
import ZzimAptItem from '@/components/main/ZzimAptItem.vue';
...
<template>
  <div class="w-full sm:w-96 h-screen flex flex-col bg-white">
    <header class="flex items-center justify-between bg-white px-4 py-2">
      ...
      <h2 class="text-surface-700">관심 아파트</h2>
      <button @click="close">
    ...
    <div class="flex bg-white gap-2 flex-1 overflow-y-auto">
      ...
      <div v-else-if="zzimList.length === 0" class="flex items-center justify-center w-full">
        <span class="text-surface-500"> 관심 아파트가 없습니다.</span>
      </div>
      <div v-else class="w-full">
        <ZzimAptItem v-for="(item, i) in zzimList" :key="item.zzimId" :item="item" :order="i + 1" />
      </div>
    </div>
  </div>
</template>
